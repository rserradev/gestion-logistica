...
from queries.query_abastecimiento_tienda import get_abastecimiento_tienda, get_traspasos_tienda
...
def export_traspasos_tienda(fecha_inicio: str, fecha_fin: str, ruta_salida="output/traspasos_tienda.csv"):
    engine = get_engine('SCP') # Obtener engine de SCP
    df_total = pd.DataFrame() # DataFrame acumulador
    dia_actual = fecha_inicio
    ...
    while dia_actual <= fecha_fin:
        ...
        query = get_traspasos_tienda(desde, hasta)
        df_dia = pd.read_sql(query, engine)
